...
load_dotenv()  # Load .env jika kamu jalankan secara lokal
...
# Endpoint untuk verifikasi webhook dari Meta (GET)
@app.get("/webhook")
async def verify_webhook(request: Request):
    params = dict(request.query_params)
    if params.get("hub.mode") == "subscribe" and params.get("hub.verify_token") == VERIFY_TOKEN:
...
# Endpoint untuk menerima pesan dari WhatsApp (POST)
@app.post("/webhook")
async def receive_message(data: dict):
    try:
        entry = data['entry'][0]
        ...
        if messages:
            ...
            if 'text' in message:
                ...
                # Balasan dari Gemini
                ai_response = gemini_reply(text)
                print("Balasan Gemini:", ai_response)

                # Kirim balasan ke WhatsApp
                send_whatsapp_message(sender, ai_response)
            else:
                print("Pesan bukan teks. Tidak diproses.")
    except Exception as e:
        print("Error:", e)
    return {"status": "ok"}
...
def send_whatsapp_message(to, message):
    ...
    print("Respon WhatsApp:", res.status_code, res.text)


# Fungsi untuk dapatkan balasan dari Gemini
def gemini_reply(user_message):
    GEMINI_API_KEY = os.getenv("GEMINI_API_KEY")
